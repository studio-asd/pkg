...
func TestCollectStructs(t *testing.T) {
	...
	if err := testPG.BulkInsert(ctx, "structs", []string{"name", "score", "slice", "nullable", "bytes"}, []any{
	...
	got, err := CollectStructs[testStruct](ctx, testPG, query)
	if err != nil {
		t.Fatal(err)
	}
	...
	gotp, err := CollectStructPtrs[testStruct](ctx, testPG, query)
	if err != nil {
		t.Fatal(err)
	}
