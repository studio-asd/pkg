...
func CollectStructs[T any](ctx context.Context, db *Postgres, query string, args ...any) ([]T, error) {
	scanner := structScannerForType[T]()
	var ts []T
	err := db.RunQuery(ctx, query, func(rows *RowsCompat) error {
		if !db.IsPgx() {
			var s T
			...
			s, err := pgx.RowToStructByPos[T](rows.pgxRows)
			if err != nil {
				return err
			}
			ts = append(ts, s)
...
func CollectStructPtrs[T any](ctx context.Context, db *Postgres, query string, args ...any) ([]*T, error) {
	scanner := structScannerForType[T]()
	var ts []*T
	err := db.RunQuery(ctx, query, func(rows *RowsCompat) error {
		if !db.IsPgx() {
			...
			s, err := pgx.RowToAddrOfStructByPos[T](rows.pgxRows)
			if err != nil {
				return err
			}
			ts = append(ts, s)
